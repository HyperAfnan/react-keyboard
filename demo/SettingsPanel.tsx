...
export function SettingsPanel({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "settings-backdrop fixed inset-0 z-40 bg-black/40 backdrop-blur-[2px]",
          isOpen && "is-open"
        )}
        onClick={onClose}
        aria-hidden="true"
      ...
      <aside
        ref={drawerRef}
        className={cn(
          "settings-drawer fixed top-0 right-0 z-50 flex h-screen w-[280px] flex-col overflow-y-auto bg-[#1c1c20] px-5 py-6 shadow-[-8px_0_32px_rgba(0,0,0,0.5)] border-l border-white/5 outline-none",
          isOpen && "is-open"
        )}
        role="dialog"
        aria-modal="true"
            ...
            {SOUND_PACKS.map((pack) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (soundEnabled) onSoundPackChange(pack.value);
                  }}
